...
$periodeTerakhir = $koneksi->query("SELECT id_periode FROM periode ORDER BY id_periode DESC LIMIT 1")->fetch_assoc();
$idPeriode = $periodeTerakhir['id_periode'];


$selecHasil = $koneksi->query("SELECT * FROM hasil_akhir WHERE f_id_periode='$idPeriode'");
$jumlahRows = mysqli_num_rows($selecHasil);

// Nama file JSON yang ingin dibaca
$namaFile = './admin/data.json';

// Baca isi file JSON
$dataJSON = file_get_contents($namaFile);

// Konversi JSON menjadi array
$dataArray = json_decode($dataJSON, true);
$deskripsiPeriode = $koneksi->query("SELECT * FROM periode WHERE id_periode='".$dataArray['id_periode']."' ORDER BY id_periode DESC LIMIT 1")->fetch_assoc();
?>
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <p class="card-text mt-2" style="font-family: 'Public Sans', sans-serif;">
                                    <?=substr("Hasil Seleksi Penerima Beasiswa", 0, 200);?></p>
                    ...
                    <div class="col-md-4 mt-2">
                        <div class="card">
                            <div class="card-body">
                                <h5 class="card-title text-secondary fw-bolder"
                                    style="font-family: 'Public Sans', sans-serif;">
                                    Hasil Seleksi Penerima Beasiswa <?=$deskripsiPeriode['deskripsi']?>
                                </h5>
                                <small>
                                    <i>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                            ...
                                            <path
                                                d="M8 16A8 8 0 1 0 8 0a8 8 0 0 0 0 16zm7-8A7 7 0 1 1 1 8a7 7 0 0 1 14 0z" />
                                        </svg> <?= modifWaktu($dataArray['tanggal']);?>
                                    </i>
                                </small>
                                <p class="card-text mt-2" style="font-family: 'Public Sans', sans-serif;">
                                    <?=substr($dataArray['isi_pengumuman'].' '.$deskripsiPeriode['deskripsi'], 0, 200);?></p>
                                <a href="./pengumuman.php?per=<?=base64_encode($dataArray['id_periode']);?>">Baca
                                    Selengkapnya...</a>
                            </div>
                        </div>
                    </div>
                    <?php endif;?>
                    <?php if($jumlahRows <= 0 && mysqli_num_rows($dataPengumuman) <= 0):?>
