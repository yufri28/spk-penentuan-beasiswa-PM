...
class Verifikasi{
    ...
    public function getPelamarVerifikasi($id_rayon=null){
        return $this->db->query("SELECT * FROM `verifikasi` v JOIN data_pelamar dp ON v.f_id_pelamar=dp.id_pelamar JOIN login_pelamar lp ON dp.f_id_login = lp.id_login JOIN rayon r ON r.id_rayon = dp.f_id_rayon WHERE v.status='1' AND dp.f_id_rayon=$id_rayon;");
    }

    public function cekVerifikasiUser($id_login=null,$id_rayon=null){
        // return $this->db->query("SELECT * FROM verifikasi WHERE f_id_pelamar=$id_pelemar")->fetch_assoc();
        return $this->db->query("SELECT * FROM `verifikasi` v JOIN data_pelamar dp ON dp.id_pelamar=v.f_id_pelamar JOIN rayon r ON r.id_rayon=dp.f_id_rayon JOIN login_pelamar lp ON dp.f_id_login=lp.id_login WHERE dp.f_id_rayon=$id_rayon AND dp.f_id_login=$id_login;")->fetch_assoc();
    }
}
